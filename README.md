...
T. Transou - June 2026 - Active Reconstruction
...
This repository is an intentional public reconstruction of the SCARAG framework from retained architecture notes, implementation knowledge, and design documentation.
...
Some sections describe reconstructed functionality in this repository. Other sections are explicit reconstruction targets that define what still needs to be built. Those targets remain here by design.
...
## Capability Matrix
| Capability | Current public status | Current implementation surface | Reconstruction intent / next step |
|---|---|---|---|
...
| document type inference | Reconstructed | scarag/pipeline.py infer_doc_type | Expand taxonomy and profile-driven typing |
| prose chunking | Reconstructed | scarag/pipeline.py _chunk_prose | Add cohesion-aware splitting and policy-tuned chunking |
...
| thesaurus/query expansion | Reconstructed | config/synonyms.json + scarag/pipeline.py expand_query_terms | Add profile overlays, governance for term drift, and diagnostics |
| lexical retrieval | Reconstructed | scarag/pipeline.py retrieve_chunks token overlap + doc_type weighting | Tune weights and add calibration tooling |
| vector or TF-IDF retrieval | Reconstruction target | Not implemented in current public pipeline | Add TF-IDF/vector backend and score normalization path |
| hybrid reranking | Reconstruction target | Not implemented; simple lexical weighting only | Add hybrid retrieval and rerank strategies with diagnostics |
| confidence resolver | Reconstruction target | api_server.py exposes simple high/low/abstain signal, no resolver module | Build confidence framework from extraction metadata and overlays |
| lifecycle/freshness controls | Reconstruction target | No persistent lifecycle state; no freshness filtering in retrieval | Add lifecycle metadata model, freshness filters, and stateful re-ingestion |
| soft delete/re-ingestion state | Reconstruction target | No re-ingestion state store in current code | Add state path, soft delete markers, and audit timeline tooling |
...
| citation response contract | Reconstructed | docs/reference-ui-contract.md and api_server.py response fields | Expand contract tests and richer citation metadata |
| reference API | Reconstructed | api_server.py /api/health and /api/chat | Add config endpoints and diagnostic surfaces |
...
- Reconstruction tracking: docs/reconstruction-status.md
...
- The current public reconstruction uses lexical overlap retrieval.
- TF-IDF/vector retrieval and hybrid reranking remain reconstruction targets.
...
### Lifecycle and Freshness - Status: Reconstruction target
Current reconstruction:
- Basic source metadata is propagated on chunks.

Important correction:
- Persistent lifecycle state, freshness filtering, and soft-delete/re-ingestion controls are reconstruction targets and are not fully implemented in current public code.

### Confidence Assessment - Status: Reconstruction target
Current reconstruction:
- API exposes a lightweight confidence signal (high, low, abstain).

Important correction:
- The confidence framework described in SCARAG design docs is part of the reconstruction roadmap; a full confidence resolver is not yet implemented.
...
### Framework Versus Implementation Boundaries - Status: Reconstructed
...
These files are recommended next additions for reconstruction clarity.
